...
%token MAIN_FUNC Number INPUT OUTPUT WORD ADD SUB MUL DIV EXPONEN SQR SQRT CUBE SINE COSINE TANGENT LN EQUALS EQUALSTO LESS_THAN GREATER_THAN LESS_EQUALS GREATER_EQUALS NE COMA ID SEMI COLON LFBR RFBR LB RB FACTORIAL Leap_Year PRIME EVEN_ODD SUM_OF_NUMBERS IF OUTPUTWORD ELSE ELSEIF FOR WHILE SWITCH DEFAULT 
%nonassoc IF
%nonassoc ELSE
...
print       : OUTPUT LFBR ID RFBR SEMI 
            ...
            | OUTPUTWORD LFBR WORD RFBR SEMI
                    {   
                        //fprintf(yyout,"here is your direct out ");
                        int i,l;
                        l=strlen($3);
                        for(i=2;i<l;i++)
                        {
